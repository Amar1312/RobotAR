...
public class ThrowBallScript : MonoBehaviour
{
    ...
    // Private Vars
    Vector3 pointer_start_pos, pointer_end_pos, pointer_direction; // Positions
    ...
    Vector2 cursor_pos = new Vector3(); // Position of touch or mouse pointer on screen

    public Transform _spawnPoint;

    void Start()
    {
        _spawnPoint = ThrowGameManager.Instance.spawn_transform;
    ...
    void Update()
    {
        ...
        //    cursor_pos = Input.mousePosition ; // Instead of getting pixels, we are getting viewport coordinates which is resolution independent
        //}
        //else
        //{
        //    if (Input.touchCount > 0)
        //    {
        //        //cursor_pos = Camera.main.ScreenToViewportPoint(Input.GetTouch(0).position) * 1000; // Instead of getting pixels, we are getting viewport coordinates which is resolution independent
        //        cursor_pos = Input.GetTouch(0).position ; // Instead of getting pixels, we are getting viewport coordinates which is resolution independent
        //    }
        //}
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            pointer_start_pos = Input.mousePosition;
        }
        if (Input.GetMouseButtonUp(0))
        {
            OffScreen(Input.mousePosition);
        }
        ...
        //    OnHold(cursor_pos);
        //}
        //if (((Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Ended) || Input.GetMouseButtonUp(0)) && drag_active)
    ...
    //{
    //    if (drag_active)
    //    { // If we throwing something
    //        if ((Time.time - pointer_start_time) > 0.3f)
    //        { // If finger holding something to long
    ...
    void OffScreen(Vector3 cursor_pos)
    {
        ...
        //Debug.Log(pointer_start_pos + "  " + pointer_end_pos);
        ...
        if (pointer_hold_duration > 0.05f)
        { // Protect from infinite speed throw
            my_rigidbody = gameObject.AddComponent<Rigidbody>();
            ...
            // Calc throw vector
            //Vector3 vector = new Vector3(-pointer_direction.x * force_xy, -pointer_direction.y * force_xy, force_z / pointer_hold_duration);
            my_rigidbody.useGravity = true;
            Vector3 vector = (_spawnPoint.forward * force_z) + new Vector3(0, -pointer_direction.y/3, 0) ;
            ...
            ThrowGameManager.Instance?.Throw(); // Decrease amounts of throws
