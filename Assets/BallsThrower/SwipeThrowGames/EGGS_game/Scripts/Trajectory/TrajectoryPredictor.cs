...
public class TrajectoryPredictor : MonoBehaviour
{
    ...
    public void PredictTrajectory(ProjectileProperties projectile)
    {
        ...
        for (int i = 1; i < maxPoints; i++)
        {
            ...
            if (Physics.Raycast(position, velocity.normalized, out RaycastHit hit, overlap , _layer))
            {
                UpdateLineRender(i, (i - 1, hit.point));
                MoveHitMarker(hit);
                if(hit.collider.gameObject.TryGetComponent(out EggsTarget target))
                {
                    target.ScaleUp();
                    _target = target;
                }
    ...
    private void MoveHitMarker(RaycastHit hit)
    {
        ...
        // Offset marker from surface 0.025f
        float offset = 0.08f;
