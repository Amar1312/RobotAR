...
public class ThrowScript : MonoBehaviour {
    ...
    void Start() {
        _spawnPoint = Basketball_game_manager.Instance.spawn_point;
    ...
    void OffScreen(Vector3 cursor_pos) {
        ...
        { // Protect from infinite speed throw
            ...
            //Debug.Log("y value " + -pointer_direction.y/3);
            //Debug.Log("x value " + (-pointer_direction.x / force_xy));
            Vector3 vector = (_spawnPoint.forward * force_z) + new Vector3(0, -pointer_direction.y/3f, 0) + new Vector3(-pointer_direction.x / force_xy, 0, 0); // Calc throw vector
            ...
            //Basketball_game_manager.Instance.CheckGameStatus(); // Decrease amounts of throws
            Invoke(nameof(NextBall), 1f);
            if (auto_destroy)
            {
                Invoke(nameof(GameObjectDestroy), auto_destroy_timer);
            }
    ...
    {
        Destroy(gameObject); // Destroy this object after "auto_destroy_timer" secs
       // Basketball_game_manager.Instance.CheckGameStatus();
        Destroy(this); // Destroy this script to prevent controlling etc.
    }

    void NextBall()
    {
        Basketball_game_manager.Instance.NextBall();
    }
}
